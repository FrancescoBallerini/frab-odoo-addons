    ...
    with odoo.api.Environment.manage():
        with odoo.registry(db_name).cursor() as cr:
            ...
            sys_param_timeout = (
                ...
                .sudo()
                .get_param("socket_timeout_setup.timeout_value")
            )
            err = False
    ...
    def process_request(self, client, addr):
        ...
        if config_socket_timeout:
            try:
                ...
                err = (
                    "Wrong value for odoo configuration file in 'socket_timeout' "
                    "variable: %s. Value must be integer." % config_socket_timeout
                )
                _logger.warning(err)

            if not err:
                timeout_values.update({"config_socket_timeout": config_socket_timeout})
        ...
        if timeout_values:
            if (
                "config_socket_timeout" in timeout_values
                and "sys_param_timeout" in timeout_values
            ):
                ...
                "config_socket_timeout" in timeout_values
                and "sys_param_timeout" not in timeout_values
            ):
                timeout_config_or_sysparam = timeout_values["config_socket_timeout"]
            elif (
                "config_socket_timeout" not in timeout_values
                and "sys_param_timeout" in timeout_values
            ):
                timeout_config_or_sysparam = timeout_values["sys_param_timeout"]
...
odoo.service.server.WorkerHTTP.process_request = (
    WorkerHttpProcessRequestPatch.process_request
)
